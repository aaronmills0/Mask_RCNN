...
# Query image is hardcoded to have a black background.
...
def main(frame):
    ...
    query = cv.imread(f'images/mustard-reference.png')
    ...
    for j in indices:
        ...
        if is_object(query, train, roi, query_med, masked, deviation=90, num=10, thresh=40, orb=orb, bf=bf, features=True, colours=True):
    ...
    for k in range(len(r['rois'])):
        ...
        if class_names[r['class_ids'][k]] != 'dining table' and class_names[r['class_ids'][k]] != 'refrigerator':
    ...
    for i in found:
        ...
        for j in range(len(all_objs)):
            ...
            if not np.array_equal(masked, all_objs[j]): # Add condition for point cloud camera.
